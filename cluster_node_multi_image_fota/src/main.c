 ...
 */
 ...
 /*
 * CLUSTER NODE APP CORE FIRMWARE
 * Date: 03-08-2026
 */
...
/* Access external battery value from power_thread.c */
extern int32_t battery_mv_avg;
...
static void recv_cb(const void *data, size_t len, void *priv)
{
    ...
    } else if (msg->type == ESB_IPC_MSG_TYPE_REQ_BATT) {
		LOG_INF("NET Core requested fresh battery voltage");
		ipc_send_battery(battery_mv_avg);
	}
}
...
{
	struct esb_ipc_msg msg;
	msg.type = ESB_IPC_MSG_TYPE_BATT;
	msg.length = sizeof(int32_t);
	memcpy(msg.data, &battery_mv, sizeof(int32_t));
