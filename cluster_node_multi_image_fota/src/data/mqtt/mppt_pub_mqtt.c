...
#include "transport.h"
#include "id.h"
#include "mppt_pub_json.h" // this would be switched out for protobuf or other as needed

#define MODULE mppt_pub_mqtt
LOG_MODULE_REGISTER(MODULE, CONFIG_MPPT_PUB_MQTT_LOG_LEVEL);

#define MQTT_TOPIC_GET mppt_pub_mqtt_topic_get
#define MQTT_PUBLISH mppt_pub_mqtt_publish
// dt topic
#define TOPIC MPPT_PUB_MQTT_TOPIC_DT_STR
#define TOPIC_DT_LEN ((sizeof(MPPT_PUB_MQTT_TOPIC_DT_STR) + ID_LEN) + 1)
#define QOS_DT MPPT_PUB_MQTT_DT_QOS

#define DT_OBJ_TYPE mppt_pub_t
#define DT_OBJ_P_TYPE DT_OBJ_TYPE *

#define PAYLOAD_SIZE_MAX MPPT_PUB_PAYLOAD_STR_SIZE_MAX
...
int MQTT_PUBLISH(DT_OBJ_P_TYPE obj)
{
    ...
    {
        LOG_ERR("PAYLOAD_ENCODE err: %d", err);
    }
    return err;
}