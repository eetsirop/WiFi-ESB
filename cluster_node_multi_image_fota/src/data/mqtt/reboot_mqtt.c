...
#include "transport.h"
#include "id.h"
#include "reboot_json.h" // this would be switched out for protobuf or other as needed
#include "reboot_timer.h"

#define MODULE reboot_mqtt
LOG_MODULE_REGISTER(MODULE, CONFIG_REBOOT_MQTT_LOG_LEVEL);

//
#define MQTT_TOPIC_GET reboot_mqtt_topic_get
#define ON_MQTT_PUBLISH_CB reboot_on_mqtt_publish_cb
// command topic
#define TOPIC REBOOT_MQTT_TOPIC_CMD_STR
#define TOPIC_CMD_LEN ((sizeof(REBOOT_MQTT_TOPIC_CMD_STR) + ID_LEN) + 1)
#define QOS_CMD REBOOT_MQTT_CMD_QOS
#define CMD_PAYLOAD_TYPE reboot_cmd_t
#define LOG_CMD log_reboot_cmd

static char topic_cmd_str[TOPIC_CMD_LEN];

// command response topic
#define TOPIC_ERR_RSP REBOOT_MQTT_TOPIC_CMD_ERR_RSP_STR
#define QOS_CMD_RSP REBOOT_MQTT_CMD_RSP_QOS
#define CMD_RSP_TYPE reboot_cmd_resp_t
#define LOG_CMD_RSP log_reboot_cmd_resp
#define RSP_PAYLOAD_SIZE_MAX REBOOT_RESP_PAYLOAD_STR_SIZE_MAX
...
bool ON_MQTT_PUBLISH_CB(struct mqtt_helper_buf topic, struct mqtt_helper_buf payload)
{
    ...
    if (strncmp(topic.ptr, topic_cmd_str, topic.size) == 0)
    {
        ...
        if (err == 0)
        {
            ...
            {
                LOG_DBG("Rebooting in %d seconds...", cmd.delay);
                reboot_timer_start(cmd.delay);
                publish_response(cmd.res_topic, cmd.session_id, RSP_OK);
                return true;
            }
            else if (cmd.delay == 0)
            {
                LOG_DBG("Rebooting now...");
                reboot_timer_start(0);
                publish_response(cmd.res_topic, cmd.session_id, RSP_OK);
                return true;
            }
            else
        ...
        {
            LOG_ERR("%s err %d failed to decode reboot_cmd", __func__, err);
...
static int publish_response(char *rsp_topic, int session_id, const char *status)
{
    ...
    if (!(id_get(id, sizeof(id))))
    {
        // this builds a topic like: reboot/cmd/rsp/1234567890/reboot_res
        snprintf(topic_cmd_rsp_str, MQTT_RESPONSE_TOPIC_SIZE_MAX, REBOOT_MQTT_TOPIC_CMD_RSP_STR, id, rsp_topic);
    }
    else
    {
        __ASSERT(0, "Failed to get id");
