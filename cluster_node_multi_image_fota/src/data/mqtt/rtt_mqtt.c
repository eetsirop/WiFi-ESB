    ...
    if (strncmp(topic.ptr, topic_cmd_str, topic.size) == 0)
    {
        ...
        if (err == 0)
        {
            ...
            else if (strcmp(cmd.operation, "RTT") == 0)
            {
                ...
                if (ret == 0) {
                    LOG_DBG("RTT: Got response from NET core");
                    char *node_id;
                    char *latest_ipc_msg;
                    char id[ID_LEN];
                    ...
                    /* Get the latest response data received during ipc_send_and_wait */
                    size_t len;
                    const uint8_t *rx_data = (const uint8_t *)get_latest_rx_string(&len);
                    
                    /* Format binary response back to hex string for MQTT */
                    char hex_resp[128] = {0};
                    char *out_ptr = hex_resp;
                    for (int i = 0; i < len; i++) {
                        out_ptr += sprintf(out_ptr, "0x%02X%s", rx_data[i], (i == len - 1) ? "" : " ");
                    }
                    latest_ipc_msg = hex_resp;

                    publish_response(cmd.res_topic, node_id, latest_ipc_msg);
                } else {
