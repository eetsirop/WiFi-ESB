...
#if defined(CONFIG_MQTT_HELPER_HELIOGEN_PROVISION_CERTIFICATES)
#include CONFIG_MQTT_HELPER_HELIOGEN_CERTIFICATES_FILE
#endif /* CONFIG_MQTT_HELPER_HELIOGEN_PROVISION_CERTIFICATES */
#include <zephyr/logging/log.h>

LOG_MODULE_REGISTER(mqtt_helper, CONFIG_MQTT_HELPER_HELIOGEN_LOG_LEVEL);

#if defined(CONFIG_MQTT_LIB_TLS)
BUILD_ASSERT((CONFIG_MQTT_HELPER_HELIOGEN_SEC_TAG != -1), "Security tag must be configured");
#endif /* CONFIG_MQTT_LIB_TLS */

/* Define a custom MQTT_HELPER_HELIOGEN_STATIC macro that exposes internal variables when unit testing. */
#if defined(CONFIG_UNITY)
#define MQTT_HELPER_HELIOGEN_STATIC
#else
#define MQTT_HELPER_HELIOGEN_STATIC static
#endif

MQTT_HELPER_HELIOGEN_STATIC struct mqtt_client mqtt_client;
static struct sockaddr_storage broker;
static char rx_buffer[CONFIG_MQTT_HELPER_HELIOGEN_RX_TX_BUFFER_SIZE];
static char tx_buffer[CONFIG_MQTT_HELPER_HELIOGEN_RX_TX_BUFFER_SIZE];
MQTT_HELPER_HELIOGEN_STATIC char payload_buf[CONFIG_MQTT_HELPER_HELIOGEN_PAYLOAD_BUFFER_LEN];
MQTT_HELPER_HELIOGEN_STATIC K_SEM_DEFINE(connection_poll_sem, 0, 1);
static struct mqtt_helper_cfg current_cfg;
MQTT_HELPER_HELIOGEN_STATIC enum mqtt_state mqtt_state = MQTT_STATE_UNINIT;
...
MQTT_HELPER_HELIOGEN_STATIC enum mqtt_state mqtt_state_get(void)
{
	return mqtt_state;
}

MQTT_HELPER_HELIOGEN_STATIC void mqtt_state_set(enum mqtt_state new_state)
{
...
static int certificates_provision(void)
{
	...
	{
		err = tls_credential_add(CONFIG_MQTT_HELPER_HELIOGEN_SEC_TAG,
...
#if CONFIG_MQTT_HELPER_HELIOGEN_SECONDARY_SEC_TAG != -1

	if (sizeof(ca_certificate_2) > 1)
	{
		err = tls_credential_add(CONFIG_MQTT_HELPER_HELIOGEN_SECONDARY_SEC_TAG,
								 TLS_CREDENTIAL_CA_CERTIFICATE,
								 ca_certificate_2,
...
#endif /* CONFIG_MQTT_HELPER_HELIOGEN_SECONDARY_SEC_TAG != -1 */
...
MQTT_HELPER_HELIOGEN_STATIC void on_publish(const struct mqtt_evt *mqtt_evt)
{
	int err;
	...
	if (err)
	{
		...
		{
			current_cfg.cb.on_error(MQTT_HELPER_HELIOGEN_ERROR_MSG_SIZE);
		}
...
MQTT_HELPER_HELIOGEN_STATIC void mqtt_evt_handler(struct mqtt_client *const mqtt_client,
					   ...
					   struct mqtt_helper_conn_params *conn_params)
{
	...
	if (sizeof(CONFIG_MQTT_HELPER_HELIOGEN_STATIC_IP_ADDRESS) > 1)
	{
		conn_params->hostname.ptr = CONFIG_MQTT_HELPER_HELIOGEN_STATIC_IP_ADDRESS;

		LOG_DBG("%s MQTT Using static IP address for broker: %s", __func__,
				CONFIG_MQTT_HELPER_HELIOGEN_STATIC_IP_ADDRESS);
	}
	else
	{
	...
	while (addr != NULL)
	{
		if (addr->ai_addrlen == sizeof(struct sockaddr_in))
		{
			...
			broker4->sin_port = htons(CONFIG_MQTT_HELPER_HELIOGEN_PORT);
						  ...
						  struct mqtt_utf8 *will_message)
{
	...
	sec_tag_t sec_tag_list[] = {
		CONFIG_MQTT_HELPER_HELIOGEN_SEC_TAG,
#if CONFIG_MQTT_HELPER_HELIOGEN_SECONDARY_SEC_TAG > -1
		CONFIG_MQTT_HELPER_HELIOGEN_SECONDARY_SEC_TAG,
#endif
	};
	...
	tls_cfg->set_native_tls = IS_ENABLED(CONFIG_MQTT_HELPER_HELIOGEN_NATIVE_TLS);
...
#endif /* defined(CONFIG_MQTT_HELPER_HELIOGEN_PROVISION_CERTIFICATES) */
#endif /* defined(CONFIG_MQTT_LIB_TLS) */

	/*
	HELIOGEN ADDITION TO NORDIC mqtt_helper.c:

	To enable Last Will and Testiment, you need to set these items in the mqtt_client struct when connecting
	...
	*/
	...
	/* add last will and testiment message */
	LOG_DBG("HELIOGEN mqtt_helper.c: adding last will and testiment message");
	...
	mqtt_client.will_message = will_message;
	mqtt_client.will_retain = 1;

	/* END HELIOGEN mqtt_helper.c ADDITION */
	...
	if (err)
	{
		LOG_INF("mqtt_connect, error: %d", err);
		return err;
	}
	...
	if (IS_ENABLED(CONFIG_MQTT_HELPER_HELIOGEN_SEND_TIMEOUT))
	{
		struct timeval timeout = {
			.tv_sec = CONFIG_MQTT_HELPER_HELIOGEN_SEND_TIMEOUT_SEC};
		...
		else
		{
			LOG_DBG("Using send socket timeout of %d seconds",
					CONFIG_MQTT_HELPER_HELIOGEN_SEND_TIMEOUT_SEC);
		}
	}
						...
						struct mqtt_utf8 *will_message)
{
	...
	if (!mqtt_state_verify(MQTT_STATE_DISCONNECTED) && !mqtt_state_verify(MQTT_STATE_CONNECTING))
	{
		LOG_ERR("Library is in the wrong state (%s), %s required",
				state_name_get(mqtt_state_get()),
	...
	err = client_connect(conn_params, will_topic, will_message);
	if (err)
	{
		mqtt_state_set(MQTT_STATE_DISCONNECTED);
		return err;
...
MQTT_HELPER_HELIOGEN_STATIC void mqtt_helper_poll_loop(void)
{
	int ret;
...
K_THREAD_DEFINE(mqtt_helper_thread, CONFIG_MQTT_HELPER_HELIOGEN_STACK_SIZE,
